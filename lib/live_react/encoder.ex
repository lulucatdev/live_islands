defprotocol LiveReact.Encoder do
  ...
  allows LiveReact to calculate JSON patches over encoded values.
  """
...
defimpl LiveReact.Encoder, for: List do
  def encode(list, opts), do: Enum.map(list, &LiveReact.Encoder.encode(&1, opts))
end

defimpl LiveReact.Encoder, for: Map do
  def encode(map, opts) do
    Map.new(map, fn {key, value} -> {key, LiveReact.Encoder.encode(value, opts)} end)
  end
end
...
defimpl LiveReact.Encoder, for: Phoenix.HTML.Form do
  ...
  defp translate_error({msg, opts}) do
    backend = Application.get_env(:live_react, :gettext_backend, nil)
    count = opts[:count]
...
defimpl LiveReact.Encoder, for: Any do
  ...
  def encode(%{__struct__: module} = struct, _opts) do
    raise Protocol.UndefinedError,
      ...
      value: struct,
      description: """
      LiveReact.Encoder protocol must be explicitly implemented for #{inspect(module)}.
      ...
      define a custom `defimpl LiveReact.Encoder`.
      """
  end
