defmodule LiveIslands.SSR.ViteJS do
  @moduledoc """
  ...
  def render(name, props, slots) do
    render(:react, name, props, slots)
  end

  def render(framework, name, props, slots) do
    data = Jason.encode!(%{framework: framework, name: name, props: props, slots: slots})
  ...
  def vite_path(path) do
    case Application.get_env(:live_islands, :vite_host) do
      nil ->
        message = """
        Vite.js host is not configured. Please add the following to config/dev.ex

        config :live_islands, vite_host: "http://localhost:5173"

        and ensure vite.js is running
        """

        raise %LiveIslands.SSR.NotConfigured{message: message}
