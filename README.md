...
- A complex `/todo` demo app that combines React and Vue islands, server-only SSR, deferred SSR, visible/interaction hydration, intent prefetch, event replies, native LiveView forms, URL patching, JS commands, server streams, and LiveView mutations in one product workflow
...
The `/benchmarks` demo route also has an online browser probe. It measures the
current browser session with the Performance API and triggers the PDF.js + KaTeX
interaction from the page, without exposing a server-side command runner. Use it
for quick interactive diagnostics; use `make benchmark` for release evidence.
