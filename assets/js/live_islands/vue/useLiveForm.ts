...
type PathValue<
  T,
  P extends string,
> = P extends `${infer Key}[${infer _}].${infer Rest}`
  ? Key extends keyof T
...
export interface FormField<T> {
  ...
  fieldArray<K extends keyof T>(
    key: K,
  ): T[K] extends readonly (infer U)[] ? FormFieldArray<U> : never;
...
export interface FormFieldArray<T>
  extends Omit<FormField<T[]>, "field" | "fieldArray"> {
...
export interface UseLiveFormReturn<T extends object> {
  ...
  field<P extends PathsToStringProps<T>>(
    path: P,
    options?: FieldOptions,
...
export function useLiveForm<T extends object>(
  ...
  const { debouncedFn: debouncedSendChanges, isPending: isValidatingChanges } =
    debounce(sendChanges, debounceWait);

  // Create a form field for a given path
  function createFormField<V>(
    path: string,
    options: FieldOptions = {},
  ): FormField<V> {
    ...
    const existingField = fieldsForPath.find((f) =>
      deepEqual(f._options, options),
    );
    if (existingField) {
    ...
    const fieldId =
      ...
      (options.value !== undefined
        ? `_${sanitizeId(String(options.value))}`
        : "");
    ...
    const fieldIsTouched = computed(
      () => submitCount.value > 0 || touchedFields.has(path),
    );
    ...
    const isMultiCheckboxValue =
      options.type === "checkbox" && Array.isArray(fieldValue.value);
    ...
    const field: FormField<V> = {
      ...
      field<K extends keyof V>(
        ...
        return createFormField(
          subPath,
          options,
        ) as V[K] extends readonly (infer U)[]
          ? FormFieldArray<U>
          : FormField<V[K]>;
      },

      fieldArray<K extends keyof V>(
        key: K,
      ): V[K] extends readonly (infer U)[] ? FormFieldArray<U> : never {
        const subPath = path ? `${path}.${String(key)}` : String(key);
        return createFormFieldArray(
          subPath,
        ) as V[K] extends readonly (infer U)[] ? FormFieldArray<U> : never;
      },
  ...
  function createFormFieldArray<V>(path: string): FormFieldArray<V> {
    ...
    const fieldArray = {
      ...
      fieldArray<P extends string | number>(
        pathOrIndex: P,
      ): ArrayFieldArrayPath<V, P> {
        // Handle number shortcut: convert 0 to "[0]"
        if (typeof pathOrIndex === "number") {
          return createFormFieldArray(
            `${path}[${pathOrIndex}]`,
          ) as ArrayFieldArrayPath<V, P>;
        }
