...
export const useLiveUpload = (
  ...
  const createInput = (config: UploadConfig) => {
    ...
    stopSyncInput = watchEffect(() => {
      ...
      const joinEntries = (entries: UploadEntry[]) =>
        entries.map((e) => e.ref).join(",");
      ...
      input.setAttribute(
        "data-phx-done-refs",
        joinEntries(current.entries.filter((e) => e.done)),
      );
      input.setAttribute(
        "data-phx-preflighted-refs",
        joinEntries(current.entries.filter((e) => e.preflighted)),
      );
    });
  ...
  const progress = computed(() => {
    ...
    const totalProgress = allEntries.reduce(
      (sum: number, entry) => sum + (entry.progress || 0),
      0,
    );
  ...
  const submit = () => {
    ...
    if (inputEl.value) {
      ...
      inputEl.value.form?.dispatchEvent(
...
>(
  ...
  const execute = (params?: P): Promise<T> => {
    if (isLoading.value) {
      console.warn(
        `Event "${eventName}" is already executing. Call cancel() first if you want to start a new execution.`,
      );
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      live.pushEvent(eventName, params, (reply: T) => {
        ...
        if (currentToken === executionToken) {
          ...
          data.value = options?.updateData
            ? options.updateData(reply, data.value)
            : reply;
...
export const useLiveConnection = (): UseLiveConnectionReturn => {
  ...
  onUnmounted(() => {
    ...
    const refs = [openRef, closeRef, errorRef].filter(
      (ref) => ref !== null,
    ) as string[];
    if (refs.length > 0) {
      socket.off(refs);
