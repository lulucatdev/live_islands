...
const getHookSlots = (hook: LiveHook): SlotMap =>
  (hook.vue.slots || {}) as SlotMap;
...
const renderInjectionSlot = (injectorId: string, component: any) => {
  ...
  return (slotProps: Record<string, any> = {}) => {
    ...
    return h(wrapper, null, {
      default: () =>
        h(component, { ...slotProps, ...hook.vue.props }, hook.vue.slots),
    });
  };
};
...
export const syncSlots = (targetId: string | null) => {
  ...
  if (injected) {
    for (const [slotName, injectorId] of injected) {
      ...
      if (entry)
        hookSlots[slotName] = renderInjectionSlot(injectorId, entry.component);
    }
  }
};

export const registerInjector = (
  id: string,
  targetId: string,
  slotName: string,
  component: any,
