...
export const getSlots = (el: HTMLElement): SlotMap => {
  ...
  return mapValues(
    dataSlots,
    (base64) => () => h("div", { innerHTML: fromUtf8Base64(base64).trim() }),
  );
};
...
export const getHandlers = (
  ...
  for (const handlerName in handlers) {
    ...
    result[snakeCaseName] = (event) => {
      ...
      const replacedOps = parsedOps.map(
        ([op, args, ...other]: [string, any, ...any[]]) => {
...
export const getElementId = (el: HTMLElement): string | null =>
