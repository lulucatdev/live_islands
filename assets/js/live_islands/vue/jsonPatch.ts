    ...
    switch (operation.op) {
      ...
      case "copy":
        return getValueByPointer(
          document,
          (operation as MoveOperation | CopyOperation).from,
        );
      case "test":
  ...
  if (Array.isArray(obj)) {
    ...
    switch (operation.op) {
      ...
      case "upsert":
        ...
        break;
      case "move":
        const moveValue = getValueByPointer(
          document,
          (operation as MoveOperation).from,
        );
        ...
        applyOperation(document, {
          op: "remove",
          path: (operation as MoveOperation).from,
        });
        obj.splice(index, 0, moveValue);
        break;
      case "copy":
        const copyValue = getValueByPointer(
          document,
          (operation as CopyOperation).from,
        );
    ...
    switch (operation.op) {
      ...
      case "replace":
        obj[unescapedKey] = (
          operation as AddOperation | ReplaceOperation
        ).value;
        break;
      case "remove":
